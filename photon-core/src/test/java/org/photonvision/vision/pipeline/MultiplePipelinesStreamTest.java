...
public class MultiplePipelinesStreamTest {
    ...
    public void testMultiplePipelinesRunConcurrently() throws Exception {
        ...
        System.out.println(
                "MultiplePipelinesStreamTest: mats before="
                        + before
                        + ", after="
                        + after
                        + ", delta="
        ...
        int allowed =
                framesTotal * 3 + pipelineCount * 2; // allow up to 3 mats per frame plus some overhead
        assertTrue(
                delta <= allowed,
                "Unexpectedly high mat allocation during stream (delta="
                        + delta
                        + ", allowed="
                        + allowed
                        + ")");
